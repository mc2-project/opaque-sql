...
/**
 * A read-only, typed Flatbuffers buffer, along with its length. Does not own
 * its buffer.
 */
...
/**
 * A typed Flatbuffers buffer stored in untrusted memory, along with its length.
 * Owns its buffer.
 */
template <typename T> struct UntrustedBufferRef {
  UntrustedBufferRef() : buf(nullptr), len(0) {}
  UntrustedBufferRef(std::unique_ptr<uint8_t, decltype(&ocall_free)> _buf,
  ...
  BufferRefView<T> view() const { return BufferRefView<T>(buf.get(), len); }
...
template <typename T>
flatbuffers::Offset<T> flatbuffers_copy(const T *flatbuffers_obj,
  ...
  void set(const tuix::Row *row) {
    if (row != nullptr) {
      ...
      if (!v.VerifyBuffer<tuix::Row>(nullptr)) {
        throw std::runtime_error(std::string("Corrupt Row buffer of length ") +
                                 std::to_string(len));
      }
