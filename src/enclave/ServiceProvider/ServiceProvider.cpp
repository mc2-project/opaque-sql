...
void lc_check(lc_status_t ret) {
  if (ret != LC_SUCCESS) {
    ...
    throw std::runtime_error(std::string("Service provider crypto failure: ") +
                             error);
  }
}
...
void ServiceProvider::load_private_key(const std::string &filename) {
  ...
  if (private_key_file == nullptr) {
    throw std::runtime_error(
        ...
        std::string(
            "' does not exist. Set $PRIVATE_KEY_PATH to the file generated by "
  ...
  if (pkey == nullptr) {
    throw std::runtime_error("Unable to read private key from '" + filename +
                             std::string("'."));
  }
  ...
  BIGNUM *x_ec = BN_new();
  BIGNUM *y_ec = BN_new();
  if (EC_POINT_get_affine_coordinates_GFp(group, point, x_ec, y_ec, nullptr) ==
      0) {
...
void ServiceProvider::export_public_key_code(const std::string &filename) {
  ...
  file << "{";
  for (uint32_t i = 0; i < LC_ECP256_KEY_SIZE; ++i) {
    file << "0x" << std::hex << std::setfill('0') << std::setw(4)
         << int(sp_pub_key.gx[i]);
...
bool verify_mrsigner(char *signing_public_key_buf,
                     size_t signing_public_key_buf_size, uint8_t *signer_id_buf,
  ...
  res = mbedtls_pk_parse_public_key(
      &ctx, (const unsigned char *)signing_public_key_buf,
      signing_public_key_buf_size + 1);

  if (res != 0) {
    std::string err_str =
        std::string("mbedtls_pk_parse_public_key failed with ") +
  ...
  if (pk_type != MBEDTLS_PK_RSA) {
    std::string err_str =
        std::string("mbedtls_pk_get_type had incorrect type: ") +
        std::to_string(res) + std::string("\n");
    throw std::runtime_error(err_str);
  }
  ...
  res = mbedtls_rsa_export_raw(rsa_ctx, modulus, modulus_size, NULL, 0, NULL, 0,
                               NULL, 0, NULL, 0);
  if (res != 0) {
    std::string err_str = std::string("mbedtls_rsa_export failed with ") +
                          std::to_string(res) + std::string("\n");
    throw std::runtime_error(err_str);
  }
...
ServiceProvider::process_enclave_report(oe_report_msg_t *report_msg,
...
#ifdef SIMULATE
  std::cerr
      << "Not running remote attestation because executing in simulation mode"
      << std::endl;
#else
  ...
  result = oe_verify_remote_report(report_msg->report, report_msg->report_size,
                                   NULL, 0, &parsed_report);
  if (result != OE_OK) {
  ...
  if (parsed_report.identity.product_id[0] != 1) {
    throw std::runtime_error(
        std::string("identity.product_id checking failed."));
  }

  if (parsed_report.identity.security_version < 1) {
    throw std::runtime_error(
        std::string("identity.security_version checking failed."));
  }

  // 3) Validate the report data
  //    The report_data has the hash value of the report data, which is the
  //    public
  if (lc_compute_sha256(report_msg->public_key, sizeof(report_msg->public_key),
                        sha256) != 0) {
  ...
  // Encrypt shared key
  ret = public_encrypt(pkey, this->shared_key, LC_AESGCM_KEY_SIZE,
