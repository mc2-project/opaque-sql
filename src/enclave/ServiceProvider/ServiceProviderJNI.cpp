...
JNIEXPORT void JNICALL Java_edu_berkeley_cs_rise_opaque_execution_SP_Init(
  ...
  try {
    service_provider.set_shared_key(
        reinterpret_cast<uint8_t *>(shared_key_bytes));
...
Java_edu_berkeley_cs_rise_opaque_execution_SP_ProcessEnclaveReport(
    JNIEnv *env, jobject obj, jbyteArray report_msg_input) {
  (void)obj;

  jboolean if_copy = false;
  jbyte *report_msg_bytes =
      env->GetByteArrayElements(report_msg_input, &if_copy);
  oe_report_msg_t *report_msg =
      reinterpret_cast<oe_report_msg_t *>(report_msg_bytes);

  uint32_t shared_key_msg_size = 0;
  std::unique_ptr<oe_shared_key_msg_t> shared_key_msg;
  try {
    shared_key_msg = service_provider.process_enclave_report(
        report_msg, &shared_key_msg_size);
  } catch (const std::runtime_error &e) {
    jni_throw(env, e.what());
  }
