...
void filter(uint8_t *condition, size_t condition_length, uint8_t *input_rows,
  ...
  FlatbuffersExpressionEvaluator condition_eval(
      condition_buf.root()->condition());
  RowReader r(
  ...
  while (r.has_next()) {
    ...
    if (condition_result->value_type() != tuix::FieldUnion_BooleanField) {
      throw std::runtime_error(
          std::string("Filter expression expected to return BooleanField, "
                      "instead returned ") +
          std::string(
    ...
    bool keep_row =
        !condition_result->is_null() &&
        static_cast<const tuix::BooleanField *>(condition_result->value())
            ->value();
    if (keep_row) {
      w.append(row);
    }
