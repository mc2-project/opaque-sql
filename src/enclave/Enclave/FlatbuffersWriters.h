...
public:
  ...
  /** Append the given Row, marking it as a dummy. */
  void append_as_dummy(const tuix::Row *row);
...
private:
  ...
  friend class ShuffleOutputWriter;
};
...
/** Append-only container for rows wrapped in tuix::ShuffleOutputs. */
class ShuffleOutputWriter {
public:
  void clear();
  ...
  /**
   * Wrap all rows written since the last call to this method into a single ShuffleOutput destined
   * for the given partition.
   */
  void finish_shuffle_output(uint32_t destination_partition);

  /** Expose the stored shuffle outputs as a buffer. */
  UntrustedBufferRef<tuix::ShuffleOutputs> output_buffer();

  /**
   * Expose the stored shuffle outputs as a buffer. The caller takes ownership of the resulting
   * buffer.
   */
  void output_buffer(uint8_t **output_rows, size_t *output_rows_length);
...
private:
  RowWriter container{};
  std::vector<flatbuffers::Offset<tuix::ShuffleOutput>> shuffle_outputs{};
};
