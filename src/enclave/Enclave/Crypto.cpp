...
void initKeySchedule() {
  ks.reset(new KeySchedule(reinterpret_cast<unsigned char *>(shared_key),
                           SGX_AESGCM_KEY_SIZE));
}
...
void set_shared_key(uint8_t *shared_key_bytes, uint32_t shared_key_size) {
  if (shared_key_size <= 0) {
    throw std::runtime_error(
        "Attempting to set a shared key with invalid key size.");
  }
...
void encrypt(uint8_t *plaintext, uint32_t plaintext_length,
  ...
  sgx_aes_gcm_128bit_tag_t *mac_ptr =
      (sgx_aes_gcm_128bit_tag_t *)(ciphertext + SGX_AESGCM_IV_SIZE +
                                   plaintext_length);
  mbedtls_read_rand(reinterpret_cast<unsigned char *>(iv_ptr),
                    SGX_AESGCM_IV_SIZE);

  AesGcm cipher(ks.get(), reinterpret_cast<uint8_t *>(iv_ptr),
                SGX_AESGCM_IV_SIZE);
...
void decrypt(const uint8_t *ciphertext, uint32_t ciphertext_length,
  ...
  decipher.decrypt(ciphertext_ptr, plaintext_length, plaintext,
                   plaintext_length);
