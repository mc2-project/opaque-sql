...
void count_rows_per_partition(uint8_t *input_rows, size_t input_rows_length,
  ...
  output[0] = flatbuffers::GetTemporaryPointer(
      builder, tuix::CreateField(
                   builder, tuix::FieldUnion_IntegerField,
                   tuix::CreateIntegerField(builder, num_rows).Union(), true));
...
void compute_num_rows_per_partition(uint32_t limit, uint8_t *input_rows,
  ...
  while (r.has_next()) {
    ...
    uint32_t num_rows =
        static_cast<uint32_t>(static_cast<const tuix::IntegerField *>(
    ...
    output[0] = flatbuffers::GetTemporaryPointer(
        builder,
        tuix::CreateField(builder, tuix::FieldUnion_IntegerField,
...
void limit_return_rows(uint32_t limit, uint8_t *input_rows,
...
// For each partition, return a fixed number of rows (starting from the first
// row) given a limit
void limit_return_rows(uint64_t partition_id, uint8_t *limits,
                       size_t limits_length, uint8_t *input_rows,
                       size_t input_rows_length, uint8_t **output_rows,
                       size_t *output_rows_length) {
  RowReader r_limit(
      BufferRefView<tuix::EncryptedBlocks>(limits, limits_length));
  uint64_t counter = 0;
  uint32_t limit = 0;
  while (r_limit.has_next()) {
    ...
    if (counter == partition_id) {
      limit =
          static_cast<uint32_t>(static_cast<const tuix::IntegerField *>(
                                    limit_row->field_values()->Get(0)->value())
                                    ->value());
      break;
    }
    ++counter;
