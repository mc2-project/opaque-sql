...
void non_oblivious_aggregate(uint8_t *agg_op, size_t agg_op_length,
  ...
  while (r.has_next()) {
    ...
    if (prev.get() != nullptr &&
        !agg_op_eval.is_same_group(prev.get(), cur.get())) {
  ...
  if (!(count == 0 &&
        (agg_op_eval.get_num_grouping_keys() > 0 ||
         (agg_op_eval.get_num_grouping_keys() == 0 && !is_partial)))) {
    w.append(agg_op_eval.evaluate());
  }
