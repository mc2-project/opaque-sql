...
void RowWriter::append_as_dummy(const tuix::Row *row) {
  rows_vector.push_back(flatbuffers_copy_row_as_dummy(row, builder, false));
  total_num_rows++;
  maybe_finish_block();
}
...
void ShuffleOutputWriter::clear() {
  container.clear();
  shuffle_outputs.clear();
}

void ShuffleOutputWriter::append(const tuix::Row *row) {
  container.append(row);
}

void ShuffleOutputWriter::finish_shuffle_output(uint32_t destination_partition) {
  shuffle_outputs.push_back(tuix::CreateShuffleOutput(
      container.enc_block_builder, destination_partition, container.finish_blocks()));
}

UntrustedBufferRef<tuix::ShuffleOutputs> ShuffleOutputWriter::output_buffer() {
  container.enc_block_builder.Finish(
    tuix::CreateShuffleOutputsDirect(container.enc_block_builder, &shuffle_outputs));
  ...
  UntrustedBufferRef<tuix::ShuffleOutputs> buffer(
    std::move(buf), container.enc_block_builder.GetSize());
  return buffer;
}

void ShuffleOutputWriter::output_buffer(uint8_t **output_rows, size_t *output_rows_length) {
  auto result = output_buffer();
  *output_rows = result.buf.release();
  *output_rows_length = result.len;
}
